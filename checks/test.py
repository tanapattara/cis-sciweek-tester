...
def check_answer(list_of_file):
    ...
    i = 1
    titlebar = f'testing...'
    with alive_bar(len(list_of_file), title=titlebar) as bar: 
        for team_path in list_of_file:
            bar() 
...
def list_students(path_store="./input/"):
    students = list(glob.glob(os.path.join(path_store,"*")))
    return students


list_of_input = list_students()
check_answer(list_of_input)